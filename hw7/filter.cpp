...
std::shared_ptr<Filter> Filter::GetFilterByStrategy(const string& FilterStrategy,float r ){
    if (FilterStrategy=="-box_filter")
    {
        return std::make_shared<BoxFilter>(r);
    }else if (FilterStrategy=="-tent_filter")
    {
        return std::make_shared<TentFilter>(r);
    }else if (FilterStrategy=="-gaussian_filter")
    {
        return std::make_shared<GaussianFilter>(r);
    }
    assert(0);
    return nullptr;
