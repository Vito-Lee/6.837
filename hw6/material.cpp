...
Material* Checkerboard::_getMaterial(const Vec3f &wsCoord) const {
    ...
    auto index = int(std::fabs(std::floor(tsp.x()) + std::floor(tsp.y()) + std::floor(tsp.z()))) % 2;
    return _m[index];
}
...
float Marble::_noise(const Vec3f &tsCoord) const {
    return std::sin(_freq * tsCoord.x() + _ampl * Noise::_noise(tsCoord));
}
...
float Wood::_noise(const Vec3f &tsCoord) const {
    ...
    s = fabsf(s);
    return sign * (1 - std::pow(1 - s, _freq));
}
